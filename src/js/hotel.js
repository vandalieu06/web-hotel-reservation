...
function searchBoxSection(e) {
  ...
  let dateCheckIn = document.getElementById("searchCheckIn")?.value;
  let dateCheckOut = document.getElementById("searchCheckOut")?.value;
  let countGuests =
    parseInt(document.getElementById("searchGuests")?.value) || 0;
...
function displayRooms() {
  ...
  const requiredGuests = parseInt(initialData.guests) || 0;

  const filteredRooms = rooms.filter(
    (room) => room.maxguest === requiredGuests,
  );
  ...
  document.querySelectorAll(".btn-reserva").forEach((btn) => {
    btn.addEventListener("click", (e) => {
      ...
      let datos = getDataLocalStorage(TYPESDB.INIT);
